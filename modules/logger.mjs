export default class Logger {
    ...
    static log(level, message, messageColor = Logger.colors.fgWhite) {
        const reset = Logger.colors.reset;

        const levelColors = {
            "INFO": Logger.colors.fgCyan,
            "SUCCESS": Logger.colors.fgGreen,
            "WARN": Logger.colors.fgYellow,
            "ERROR": Logger.colors.fgRed,
            "DEBUG": Logger.colors.bright + Logger.colors.fgBlack
        };
        const prefixColor = levelColors[level] || Logger.colors.fgWhite;
        ...
        const prefix = `${prefixColor}${Logger.displayDate()}[${level}] `;

        const out = lines.map(line => `${prefix}${messageColor}${line}${reset}`).join('\n');
        console.log(out);
    }
